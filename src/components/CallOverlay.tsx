...
export function CallOverlay() {
  ...
  if (incomingCall && !activeCall) {
    return (
      <div className="fixed top-4 right-4 z-50 bg-white/80 dark:bg-black/80 backdrop-blur-2xl border border-black/5 dark:border-white/10 shadow-2xl rounded-2xl p-4 w-72 flex flex-col gap-4 animate-in slide-in-from-top-4">
  ...
  if (activeCall) {
    ...
    return (
      <div className={`fixed z-50 overflow-hidden bg-white/80 dark:bg-black/80 backdrop-blur-2xl border border-black/5 dark:border-white/10 shadow-2xl flex flex-col ${(isConnected && isVideo) ? 'inset-0 md:inset-auto md:bottom-6 md:right-6 md:w-[480px] md:h-[360px] md:rounded-2xl' : 'bottom-4 right-4 rounded-2xl p-4 w-72'}`}>
        {isConnected ? (
          isVideo ? (

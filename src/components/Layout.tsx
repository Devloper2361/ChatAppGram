...
export default function Layout() {
  ...
  useEffect(() => {
    ...
    socket.on("profile_updated", (updatedUser) => {
      setConversations(prev => prev.map(c => 
        c.user.id === updatedUser.id ? { ...c, user: { ...c.user, ...updatedUser } } : c
      ));
  ...
  return (
    <div className="flex h-screen bg-background text-foreground overflow-hidden">
      {/* Sidebar */}
      <div className={`flex flex-col w-full md:w-80 border-r bg-muted/10 shrink-0 ${isMessageView && location.pathname !== '/profile' ? 'hidden md:flex' : 'flex'}`}>
        <div className="p-4 border-b flex items-center justify-between gap-2 shadow-sm cursor-pointer hover:bg-muted/30 transition-colors" onClick={() => navigate('/profile')}>
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <div className="flex flex-col overflow-hidden">
               ...
               <span className="text-xs text-muted-foreground truncate">My Profile</span>
            </div>
          </div>
          <Button variant="ghost" size="icon" onClick={(e) => { e.stopPropagation(); logout(); }}>
                  ...
                  >
                    ...
                    <div className="flex-1 overflow-hidden">
                      <div className="flex justify-between items-baseline">
                        <div className={`font-medium truncate ${isSelected ? '' : 'text-foreground'}`}>{c.user.displayName || c.user.username}</div>
                      ...
                      <div className={`text-sm flex items-center gap-1 truncate ${isSelected ? 'text-primary-foreground/80' : 'text-muted-foreground'}`}>
                        ...
                        <span className="truncate">
                           {c.lastMessage?.content || (c.lastMessage?.imageUrl ? "📷 Image" : "")}
                        </span>
                      </div>
                    </div>
                    {c.unreadCount > 0 && (
  ...
  className={`flex-1 flex flex-col min-w-0 ${
    location.pathname === "/profile"
      ? "flex"
      : (!isMessageView ? "hidden md:flex" : "flex")
  }`}
>
        <Outlet />
      </div>
